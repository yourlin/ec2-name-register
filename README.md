...
```bash
ec2-name-register$ sam build --use-container
```

The SAM CLI installs dependencies defined in `ec2_change_name/requirements.txt`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
ec2-name-register$ pip install -r tests/requirements.txt --user
# unit test
ec2-name-register$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
ec2-name-register$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name ec2-name-register
```
